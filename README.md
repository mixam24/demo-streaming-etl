...
1. Provision the Debezium Postgres CDC Source Connector. Select **Data integration > Connectors** from the left-hand menu, then search for the connector. Select the tile. Use the table below to configure it:
   ...
   | Kafka API Key                     | _get from `env.sh`_                    |
   | Kafka API Secret                  | _get from `env.sh`_                    |
   | Database hostname                 | _get from Terraform output_            |
   | Database port                     | `5432`                                 |
...
1. Create the Debezium Mysql CDC Source Connect by searching for it and selecting its tile. Configure it using the following settings.
   ...
   | --------------------------------- | ----------------------------------------------- |
   | Kafka Cluster Authentication mode | `KAFKA_API_KEY`                                 |
   | Kafka API Key                     | _copy from `env.sh`_                            |
   | Kafka API Secret                  | _copy from `env.sh`_                            |
   | Database hostname                 | _get from Terraform output_                     |
   | Database port                     | `3306`                                          |
...
1. Use the following statements to consume `customers` records.
   ...
   ```sql
   CREATE STREAM customers_stream WITH (KAFKA_TOPIC='mysql.customers.customers', KEY_FORMAT='JSON', VALUE_FORMAT='JSON_SR');
   ```
...
1. Verify `demographics_stream` stream is populated correctly and then hit **Stop**.
   ...
   ```sql
   SELECT * FROM demographics_stream;
   ```
